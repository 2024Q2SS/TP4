...
# Lista de archivos CSV generados por los distintos valores de omega
file_list = glob.glob("../DM/2_1/output_omega_*.csv")
...
# Graficar la evolución de la amplitud máxima en el tiempo para cada omega
...
for idx, filename in enumerate(sorted(file_list)):
    ...
    omega_amplitudes[omega] = max_amplitude

    # Graficar la evolución del módulo de la amplitud máxima en el tiempo
    plt.plot(t, max_amplitude_over_time, label=f"Omega = {omega}", color=colors[idx])

plt.xlabel("Tiempo (s)")
plt.ylabel("Amplitud máxima |r| (m)")
plt.title("Evolución de la Amplitud Máxima en el Tiempo para Distintas Omega")
plt.legend()
plt.grid(True)
plt.show()
...
omegas = list(map(int, omega_amplitudes.keys()))
amplitudes_max = list(omega_amplitudes.values())
plt.plot(omegas, amplitudes_max, marker="o")

plt.xlabel("Omega (rad/s)")
plt.ylabel("Amplitud máxima |r| (m)")
plt.title("Amplitud Máxima en Función de Omega")
plt.grid(True)
plt.show()
